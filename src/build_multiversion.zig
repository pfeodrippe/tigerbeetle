...
pub fn main(process_init: std.process.Init) !void {
    ...
    const tmp_dir = try std.Io.Dir.cwd().createDirPathOpen(
        stdx.process_io,
        tmp_dir_path,
        .{},
    );
    defer {
        tmp_dir.close(stdx.process_io);
        std.Io.Dir.cwd().deleteTree(stdx.process_io, tmp_dir_path) catch {};
    }
    ...
    const stat = try shell.cwd.statFile(stdx.process_io, cli_args.output, .{});
...
fn build_multiversion_body(shell: *Shell, options: struct {
    ...
    if (builtin.os.tag != .windows) {
        ...
        defer old_current_release_fd.close(stdx.process_io);
        try old_current_release_fd.setPermissions(stdx.process_io, .executable_file);
    }
    ...
    const old_current_release_size: u32 = @intCast(
        (try shell.cwd.statFile(stdx.process_io, old_current_release_output_name, .{})).size,
    );
    ...
    | {
        const past_name = try shell.fmt("{s}/tigerbeetle-past-{f}-{s}", .{
            options.tmp_path,
    ...
    defer body_file.close(stdx.process_io);
    ...
    for (
        ...
        const contents = try shell.cwd.readFileAlloc(
            ...
            past_name,
            shell.arena.allocator(),
            .limited(@as(usize, size) + 1),
        );
        try body_file.writePositionalAll(stdx.process_io, contents, offset);
    }
...
fn macos_universal_binary_build(
    ...
    for (binaries, binary_headers) |binary, *binary_header| {
        const binary_size: u32 = @intCast(
            (try shell.cwd.statFile(stdx.process_io, binary.path, .{})).size,
        );
    ...
    const output_file = try shell.project_root.createFile(stdx.process_io, output_path, .{
        .exclusive = true,
        .permissions = .executable_file,
    });
    defer output_file.close(stdx.process_io);
    var output_buffer: [64 * 1024]u8 = undefined;
    var output_writer = output_file.writer(stdx.process_io, &output_buffer);
    ...
    try output_writer.interface.writeAll(std.mem.asBytes(&fat_header));

    assert(@sizeOf(std.macho.fat_arch) == 20);
    try output_writer.interface.writeAll(std.mem.sliceAsBytes(binary_headers));

    try output_writer.seekTo(alignment);
    ...
    for (binaries, binary_headers) |binary, binary_header| {
        ...
        try output_writer.seekTo(@byteSwap(binary_header.offset));
        try output_writer.interface.writeAll(binary_contents);
    }
