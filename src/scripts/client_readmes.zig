...
pub fn test_freshness(
    ...
    var ctx = Context{
        ...
        .buffer = std.array_list.Managed(u8).init(arena.allocator()),
        .docs = docs,
...
const Context = struct {
    ...
    fn read_section(ctx: *Context, section_name: []const u8) []const u8 {
        var section_content = std.array_list.Managed(u8).init(ctx.arena);
        const section_start =
    ...
    fn print(ctx: *Context, comptime fmt: []const u8, args: anytype) void {
        ctx.buffer.print(fmt, args) catch @panic("OOM");
    }
