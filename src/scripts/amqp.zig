...
fn run_protocol_test(gpa: std.mem.Allocator, options: struct { host: stdx.net.Address }) !void {
...
fn run_serialization_test(
    ...
    options: struct { host: stdx.net.Address },
) !void {
    var context: AmqpContext = undefined;
    try context.init(gpa);
    defer context.deinit(gpa);
...
const AmqpContext = struct {
    ...
    pub fn connect(self: *AmqpContext, host: stdx.net.Address) !void {
        assert(!self.busy);
        self.busy = true;
...
const VSRContext = struct {
    ...
    pub fn init(self: *VSRContext, gpa: std.mem.Allocator, time: vsr.time.Time, port: u16) !void {
        ...
        const address = try stdx.net.Address.parseIp4("127.0.0.1", port);
...
const TmpRabbitMQ = struct {
    ...
    pub fn init(
        ...
        const host: stdx.net.Address = host: {
            ...
            while (lines.next()) |line| {
                ...
                break :host try stdx.net.Address.parseIp4("127.0.0.1", port);
            }
            try testing.expect(false);
            unreachable;
        };
