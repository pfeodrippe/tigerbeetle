...
pub fn command_inspect(
    ...
    stdout_writer.interface.flush() catch |err| {
        if (stdout_writer.err) |write_err| switch (write_err) {
            error.BrokenPipe => return,
            else => return err,
        } else return err;
    };
...
fn print_tree_schema(
    ...
    const key_size = comptime std.fmt.comptimePrint(
        "{f}",
        .{stdx.fmt_int_size_bin_exact(@sizeOf(Tree.Table.Key))},
    );
    const value_size = comptime std.fmt.comptimePrint(
        "{f}",
    ...
    fn inspect_wal(inspector: *Inspector, output: *std.Io.Writer) !void {
        ...
        for (std.mem.bytesAsSlice(vsr.Header.Prepare, headers_buffer), 0..) |*wal_header, slot| {
            ...
            for (group_by.groups()) |group| {
                ...
                var label_writer = std.Io.Writer.fixed(&label_buffer);
                try label_writer.writeByte(if (group.is_set(0)) mark else '_');
                try label_writer.writeByte(if (group.is_set(1)) mark else '_');
                try label_writer.print("{:_>4}: ", .{slot});

                try print_struct(output, label_writer.buffered(), &.{
    ...
    fn inspect_replies(
        ...
        for (&entries.headers, &entries.sessions, 0..) |*session_header, session, slot| {
            ...
            for (group_by.groups()) |group| {
                ...
                var label_writer = std.Io.Writer.fixed(&label_buffer);
                try label_writer.print("{:_>2}: ", .{slot});
                try label_writer.writeByte(if (group.is_set(0)) header_mark else '_');
                try label_writer.writeByte(if (group.is_set(1)) header_mark else '_');
                try label_writer.writeAll(" header");
                try print_struct(output, label_writer.buffered(), header);
            }
        }
    }
    ...
    fn inspect_grid(inspector: *Inspector, output: *std.Io.Writer, superblock_copy: ?u8) !void {
        ...
        ,
            .{
                ...
                free_set.highest_address_acquired(),
                stdx.fmt_int_size_bin(superblock.vsr_state.checkpoint
                    .free_set_blocks_acquired_size),
    ...
    if (Element: {
        ...
        if (Element == u8) {
            ...
            for (value[0..], 0..) |*item, index| {
                var label_writer = std.Io.Writer.fixed(&label_buffer);
                try label_writer.print("{s}[{}]", .{ label, index });
                try print_struct(output, label_writer.buffered(), item);
            }
            return;
        }
    }
            ...
            inline for (StateMachine.Forest.tree_infos) |tree_info| {
                if (metadata.tree_id == tree_info.tree_id) {
                    for (
                        ...
                        var label_writer = std.Io.Writer.fixed(&label_buffer);
                        try label_writer.print("{s}[{}]", .{ tree_info.tree_name, i });
                        if (comptime is_composite_key(tree_info.Tree.Table.Value)) {
                            try label_writer.writeAll(": ");
                            try print_struct(
                                writer,
                                label_writer.buffered(),
                                &.{ value.field, value.timestamp },
                            );
                        } else {
                            try print_struct(writer, label_writer.buffered(), value);
                        }
                    }
                    break;
                }
        ...
        if (operation_schema.operation == header.operation) {
            ...
            if (body_size == 0) {
                ...
                for (std.mem.bytesAsSlice(
                    ...
                    var label_writer = std.Io.Writer.fixed(&label_buffer);
                    try label_writer.print("events[{}]: ", .{i});
                    try print_struct(output, label_writer.buffered(), event);
                }
            } else {
                try output.print(
        ...
        if (operation_schema.operation == header.operation) {
            ...
            if (body_size == 0) {
                ...
                for (std.mem.bytesAsSlice(
                    ...
                    var label_writer = std.Io.Writer.fixed(&label_buffer);
                    try label_writer.print("results[{}]: ", .{i});
                    try print_struct(output, label_writer.buffered(), result);
                }
            } else {
                try output.print(
...
fn free_block(
    ...
    const block_slice: []align(constants.sector_size) u8 = block;
