...
test "clock: fuzz test" {
    ...
    log.info("maximum={}\n", .{
        std.Io.Duration.fromNanoseconds(@as(i64, @intCast(max_clock_offset))),
    });
    log.info("minimum={}\n", .{
        std.Io.Duration.fromNanoseconds(@as(i64, @intCast(min_clock_offset))),
    });
    log.info("\nabsolute synchronization errors between clocks:\n", .{});
    log.info("maximum={}\n", .{
        std.Io.Duration.fromNanoseconds(@as(i64, @intCast(max_sync_error))),
    });
    log.info("minimum={}\n", .{
        std.Io.Duration.fromNanoseconds(@as(i64, @intCast(min_sync_error))),
    });
