...
expect_contains "$classify_repair_budget_output" "name=RepairBudgetJournal.init body-class=native-only live-path=native-patch-candidate reason=errdefer-cleanup boundary=restricted-vm-candidate guidance=widen-restricted-vm"
expect_contains "$classify_repair_budget_output" "name=RepairBudgetGrid.init body-class=native-only live-path=native-patch-candidate reason=errdefer-cleanup boundary=restricted-vm-candidate guidance=widen-restricted-vm"
echo "repair_budget cleanup frontier classify proof: OK"
